...
$stmt = $pdo->prepare('UPDATE brtalk_user SET time = :time, typing = :typing WHERE user_id = :user_id LIMIT 1');
...
/* Lista clientes */
$stmt = $pdo->prepare('SELECT client_id, user_id, name, email, status, typing FROM brtalk_client WHERE time > :time AND status != :status ORDER BY client_id ASC');
...
/* lista mensagens */
$stmt = $pdo->prepare('SELECT client_id, user_id, message FROM brtalk_message WHERE user_id = :user_id AND type = :type ORDER BY message_id ASC');
$stmt->bindValue('user_id', $user['user_id']);
...
$stmt = $pdo->prepare('DELETE FROM brtalk_message WHERE user_id = :user_id AND type = :type');
$stmt->bindValue('user_id', $user['user_id']);
$stmt->bindValue('type', 1);
$stmt->execute();
