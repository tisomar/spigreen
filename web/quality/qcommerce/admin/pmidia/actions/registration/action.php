...
if ($request->query->has('id')) {
    ...
    if ($request->query->get('context') == ProdutoPeer::OM_CLASS) {
        if ($_classQuery::create()->filterByProdutoId($request->query->get('reference'))->count() >= Config::get('produto_limite_imagens')) {
            $session->getFlashBag()->add('error', sprintf('O limite permitido por produto &eacute; %d imagens.', Config::get('produto_limite_imagens')));
...
$cores = ProdutoVariacaoAtributoQuery::create()
    ->useProdutoAtributoQuery()
        ->filterByProdutoId($request->query->get('reference'))
