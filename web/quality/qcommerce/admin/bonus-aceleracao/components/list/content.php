...
<div class="table-responsive">
    <table width="100%" cellpadding="0" cellspacing="0" border="0" class="table table-hover table-striped">
        ...
        foreach ($pager->getResult() as $object) { /* @var $object ParticipacaoResultado */
            $linkPreview = get_url_admin() . '/bonus-aceleracao-preview/list?participacao_resultado_id=' . $object->getId();
            $linkRelatorio = get_url_admin() . '/bonus-aceleracao-relatorio/list?participacao_resultado_id=' . $object->getId();
            ?>
            <tr>
                ...
                <td class="text-right" data-title="Ações">
                    <div class="btn-group">
                        ...
                        <?php if ($object->getStatus() == ParticipacaoResultado::STATUS_PREVIEW) :  ?>    
                            ...
                            <li><a title="Cancelar" href="javascript:void(0);" class="confirmacao" data-href="<?php echo get_url_admin() . '/bonus-aceleracao/confirmar' ?>" data-id="<?php echo $object->getId() ?>" data-message="Você tem certeza de que realmente deseja confirmar esta participação nos resultados?"><span class="icon-check"></span> Confirmar</a></li>
                        <?php elseif ($object->getStatus() == ParticipacaoResultado::STATUS_DISTRIBUIDO) :  ?>         
                        ...
                        <?php if (in_array($object->getStatus(), array(ParticipacaoResultado::STATUS_AGUARDANDO_PREVIEW, ParticipacaoResultado::STATUS_PROCESSANDO_PREVIEW, ParticipacaoResultado::STATUS_PREVIEW, ParticipacaoResultado::STATUS_AGUARDANDO))) :  ?>
                            <li><a title="Cancelar" href="javascript:void(0);" class="confirmacao" data-href="<?php echo get_url_admin() . '/bonus-aceleracao/cancelar' ?>" data-id="<?php echo $object->getId() ?>" data-message="Você tem certeza de que realmente deseja cancelar esta participação nos resultados?"><span class="icon-remove"></span> Cancelar</a></li>
                        <?php endif ?>
