...
if ($container->getRequest()->query->has('exportar')) :
    ...
    $content = 'Cliente;Valor;Graduacao;Observacao;Percentual' . PHP_EOL;
    ...
    $filename = sprintf('bonus_desempenho_preview_%s.csv', date('Y-m-d H-i-s'));
...
if ($container->getRequest()->query->has('pdf')) :
    ...
    foreach ($objects as $object) : /* @var $object DistribuicaoCliente */
        ...
        $bonus = number_format($object->getTotalPontos(), '2', ',', '');
        $dados .=
            "<tr>
                ...
                <td data-title='Observacao'> {$object->getObservacao()} </td>
                <td data-title='Percentual'> {$object->getPercentual()} </td>
            </tr>";
    endforeach;
    ...
    $html = "
        ...
        <span  style='font-family:arial; text-align:center'> 
            <h2>Relatório Bônus Desempenho</h2><br>
        </span>
    ...
    $dompdf->stream('preview_bonus_desempenho.pdf');
    exit();
endif;
