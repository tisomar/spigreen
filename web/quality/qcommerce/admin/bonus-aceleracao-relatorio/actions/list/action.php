...
if ($container->getRequest()->query->has('exportar')) :
    ...
    $content = 'Cliente;Valor;Graduacao;Periodo' . PHP_EOL;
    ...
    $filename = sprintf('bonus_aceleracao_relatorio_%s.csv', date('Y-m-d H-i-s'));
...
if ($container->getRequest()->query->has('pdf')) :
    ...
    foreach ($objects as $object) : /* @var $object DistribuicaoCliente */
        ...
        $dados .=
            "<tr>
                ...
                <td data-title='Periodo'> {$object->getObservacao()} </td>
            </tr>";
    endforeach;
    ...
    $html = "
        ...
        <span  style='font-family:arial; text-align:center'> 
            <h2>Relatório Bônus Aceleração</h2><br>
        </span>
    ...
    $dompdf->stream('relatorio_bonus_aceleracao.pdf');
    exit();
endif;
