...
$sql = "
    SELECT
        ...
        cli.cpf CPF,
        ped.NUMERO_NOTA_FISCAL NOTA_FISCAL,
        ped.CODIGO_RASTREIO CODIGO_RASTREIO,
        ped.TRANSPORTADORA_NOME TRANSPORTADORA,
        ...
        CASE WHEN ped.FRETE = 'correios_04162' THEN 'Sedex'
             WHEN ped.FRETE = 'correios_04669' THEN 'PAC'
             WHEN ped.FRETE = 'transportadora' THEN 'Transportadora'
             WHEN ped.FRETE = 'retirada_loja' THEN 'Retirada na Loja'
             WHEN ped.FRETE = 'frete_gratis' THEN 'Frete Grátis'
             WHEN ped.FRETE = 'gollog' THEN 'Gollog'
             ELSE ''
        END FORMA_ENTREGA
      FROM qp1_pedido ped
...
// QUERY DIMINUINDO DO VALOR DE DESCONTO COMO ERA FEITO ANTERIORMENTE CADA VEZ QUE O PEDIDO ERA PAGO COM BONUS, NO MES DE SETEMBRO DE 2020 
// PAGAMENTO COM PONTOS SE TORNOU FORMA DE PAGAMENTO, E NAO DESCONTO COMO ERA TRATADO ANTEIORMENTE.
// $sql = "
//     SELECT
//         ped.ID PEDIDO_ID,
//         ped.CREATED_AT DATA,
//         (ped.VALOR_ITENS +
//          ped.VALOR_ENTREGA -
//          COALESCE((SELECT SUM(forma.VALOR_DESCONTO)
//                      FROM qp1_pedido_forma_pagamento forma
//                     WHERE forma.PEDIDO_ID = ped.ID
//                       AND forma.STATUS <> 'CANCELADO'), 0) -
//          COALESCE((SELECT SUM(des.VALOR_DESCONTO)
//                      FROM qp1_desconto_pagamento_pontos des
//                     WHERE des.PEDIDO_ID = ped.ID),0)
//         ) VALOR_TOTAL,
//         IF(cli.CNPJ IS NULL, cli.NOME, cli.RAZAO_SOCIAL) CLIENTE,
//         IF(ped.STATUS <> 'ANDAMENTO',
//            ped.STATUS,
    ...
    <div class="table-responsive">
        <table cellpadding="0" cellspacing="0" border="0" class="table table-striped datatables">
            ...
            <tr>
                ...
                <th width="12%">Forma Entrega</th>
                <th width="12%">Transportadora</th>
                <th width="12%">Código rastreio</th>
                <th width="12%">N° NF</th>
            ...
            while ($result = $stmt->fetch(PDO::FETCH_OBJ)) :
                ...
                $centroDistribuicao = $pedido->getCentroDistribuicao();
                ?>
                <tr>
                    ...
                    <td data-title="Forma de entrege">
                        <?= $result->FORMA_ENTREGA ?>
                    </td>
                    <td data-title="Transportadora">
                        <?= $result->TRANSPORTADORA ?>
                    </td>
                    <td data-title="Código de rastreio">
                        <?= $result->CODIGO_RASTREIO ?>
                    </td>
                    <td data-title="Nota fiscal">
                        <?= $result->NOTA_FISCAL ?>
                    </td>
                    <td data-title="Status">
                        <?= ucfirst(strtolower($result->STATUS)) ?>
                    </td>
                    <td data-title="Centro Distribuição">
