    ...
    SELECT 
      ...
      FROM (
            SELECT (SELECT SUM(pedite.VALOR_UNITARIO * pedite.QUANTIDADE)
                      ...
                      JOIN qp1_produto pro ON pro.ID = provar.PRODUTO_ID and pro.PLANO_ID IS NULL
                     WHERE pedite.PEDIDO_ID = ped.ID
                       AND pedite.PLANO_ID IS NULL
                    ) - COALESCE(pfp.VALOR_DESCONTO, 0) - COALESCE(ped.VALOR_CUPOM_DESCONTO, 0) VALOR_TOTAL_PEDIDO,
                    ped.VALOR_ENTREGA VALOR_ENTREGA,
...
while ($rs = $stmt->fetch(PDO::FETCH_OBJ)) :
    ...
    $totalizadores['valor_total_frete'] += $rs->VALOR_ENTREGA;
endwhile;
...
<div class="col-xs-12 col-md-12">
    <a href="<?php echo get_url_admin() ?>/relatorio/volume-recompras-plano/?exportar=1&<?php echo $url ?>&data_inicial=<?php echo $startDate->format('Y-m-d 00:00:00') ?>&data_final=<?php echo $endDate->format('Y-m-d 23:59:59') ?>"
...
<div class="col-xs-12 col-md-12 col-lg-4">
    <div class="report-printable">
        <h1 class="print">
            Relatório de volume de recompras por plano
        </h1>
        <h4 class="print">
            Período:
        ...
        <div class="row">
            ...
            <div class="col-xs-12 col-lg-6">
                <h3 class="text-right">
                    ...
                    <small class="text-muted">vendas neste período</small><br>
                    <small>Total com frete R$ <?= format_money($totalizadores['valor_total_frete'] + $totalizadores['valor_total_plano']) ?></small>
                </h3>
            </div>
        </div>
        <hr/>
        <div class="table-responsive">
            <table class="table">
                ...
                <tr>
                    ...
                    <th class="text-right text-muted">Frete</th>
                </tr>
                </thead>
                <tbody>
                <?php foreach ($dataValorTotal as $i => $data) : ?>
                    <tr>
                        ...
                        <td data-title="Valor" class="text-right">
                            <h5>R$&nbsp;<?php echo format_money($data['frete']); ?></h5>
                        </td>
                    </tr>
                <?php endforeach; ?>
                </tbody>
