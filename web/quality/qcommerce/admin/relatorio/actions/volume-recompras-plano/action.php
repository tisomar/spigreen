...
if ($container->getRequest()->query->has('exportar')) :
        ...
        SELECT 
          ...
          FROM (
                SELECT (SELECT SUM(pedite.VALOR_UNITARIO * pedite.QUANTIDADE)
                        ...
                        ) - COALESCE(ped.VALOR_CUPOM_DESCONTO, 0) VALOR_TOTAL_PEDIDO,
    ...
    $dados[] = [
        ...
        'R$ '. $totalizadores['valor_total_frete']
    ];

    $content = 'Plano;NumeroDePedidos;Valor,Frete' . PHP_EOL;
    ...
    $filename = sprintf('volume_recompras_plano__%s.csv', date('Y-m-d H-i-s'));
