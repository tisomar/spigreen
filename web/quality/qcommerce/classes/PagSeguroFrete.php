...
class PgsFrete
{
    ...
    private function _requestCurl($url, $post = null)
    {
        ...
        if (curl_exec($ch) === false) {
            // Caso tenha ocorrido algum erro no curl, então retorna-o
            $this->_result = 'Curl error - ' . curl_error($ch);
        } else {
            // Caso tudo ocorreu corretamente, retorna o resultado e salva na sessão
            $this->_result = $result;
    ...
    public function gerar($CepOrigem, $Peso, $Valor, $Destino)
    {
        ...
        $url = "https://pagseguro.uol.com.br/desenvolvedor/simulador_de_frete_calcular.jhtml?postalCodeFrom={$CepOrigem}&weight={$Peso}&postalCodeTo={$Destino}";
        ...
        if ($result[0] == 'ok') {
            ...
            return utf8_encode($this->_result);
        }
