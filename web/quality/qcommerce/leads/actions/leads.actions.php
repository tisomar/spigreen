    ...
    if (!$clienteLead instanceof ClienteDistribuidor) {
        if (strlen(preg_replace("/[^0-9]/", "", $arrClienteDistribuidor["TELEFONE_CELULAR"])) >= 10) {
            if (strlen(preg_replace("/[^0-9]/", "", $arrClienteDistribuidor["CEP"])) == 8) {
                ...
                $objClienteDistribuidor->setTipoLead('D');
                $objClienteDistribuidor->setLead(true);

                $clienteSite = ClienteQuery::create()->findOneByEmail($arrClienteDistribuidor['EMAIL']);

                if ($clienteSite) {
                    $distribuidor = $clienteSite->getClienteIndicadorId();
                    $distribuidorId = $distribuidor->getId();
                } else {
                    $distribuidor = ClienteDistribuidorPeer::getDistribuidorMaisPerto($objClienteDistribuidor->getCep());
                    if (!$distribuidor) {
                        FlashMsg::erro('Não tem nenhum distribuidor que possa te antender. Entre en contato con suporte');
                        redirect('/leads');
                    }
                    $distribuidorId = $distribuidor->getId();
                }
                ...
                if ($objClienteDistribuidor->myValidate($erros) && !$erros) {
                    ...
                    if (!$erros) {
                        ClienteDistribuidorPeer::notificarAoDistribuidorDoClientePeloNovoCadastro($objClienteDistribuidor);
                        ...
                        $conteudo = _mostrarConteudoDescricao('18', $dados);
                        $conteudo .= "<br>";
