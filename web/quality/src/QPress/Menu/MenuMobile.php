...
class MenuMobile {
     ...
     * @return  menu
     */
    public function add($title, $url, MenuMobile $children = NULL) {
        $this->items[] = array(
            'title' => $title,
            'url' => $url,
    ...
    public function render(array $attrs = NULL, $current = NULL, array $items = NULL) {
        ...
        foreach ($items as $key => $item) {
            ...
            if (is_array($item)) {
                ...
                $arrows = "";
                if ($has_children) {
                    $arrows .= ' <span class="pull-right ' . icon('chevron-down', true) . '"></span>' .
                        '<span class="pull-right ' . icon('chevron-up', true) . '"></span>';
                }

                $menu .= '<li>'
                        . '<a href="' . $item['url'] . '" class="collapsed" data-target=".'.$data_target.'" data-toggle="collapse">'
                        . $item['title']
                        . $arrows
                        . '</a>';

                if ($has_children) {
                    $ul_class = array('class' => 'second-level collapse ' . $data_target);
