...
class Menu {
     ...
     * @return  menu
     */
    public function add($title, $url, $icon = null, menu $children = NULL) {
        $this->items[] = array(
            'icon' => $icon,
            'title' => $title,
            'url' => $url,
    ...
    public function render(array $attrs = NULL, $current = NULL, array $items = NULL) {
        ...
        foreach ($items as $key => $item) {
            if (is_array($item)) {
                ...
                $ul_class = array('class' => 'acc-menu');

                if ($item['url'] == '' || $item['url'] == '#') {
                    $item['url'] = 'javascript:void(0)';
                } elseif (strpos($item['url'], 'http') === false) {
                    $item['url'] = get_url_admin() . $item['url'];
                } else {
                    $target = '_blank';
                }
    ...
    public function hasAccess($module_menu) {
        $module_menu = str_replace('/', '', $module_menu);
        return (bool) strpos(serialize($this), '/' . $module_menu . '/');
    }
