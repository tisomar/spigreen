...
namespace QPress\Menu\Categorias;
...
class TodasCategorias {
    ...
    public static function factory(array $items = NULL) {
        return new TodasCategorias($items);
    }
     ...
     * @return  menu
     */
    public function add($title, $url, TodasCategorias $children = NULL) {
        $this->items[] = array(
            'title' => $title,
            'url' => $url,
    ...
    public function render(array $attrs = NULL, $current = NULL, array $items = NULL) {
        ...
        if ($i == 1) {
            $menu = '<div' . self::attributes($attrs) . '>';
        } else {
        ...
        foreach ($items as $key => $item) {
            ...
            if (is_array($item)) {
                ...
                $menu .= ($i == 1 ? '<div class="mosaicflow__item submenu-col">' : '<li>')
                        . '<a href="' . $item['url'] . '">'
                        . $item['title']
                        . '</a>';

                if ($has_children) {
                    $ul_class = array('class' => 'list-unstyled');
